...
function insert_email($email = "", $emailtype = "") {
	...
	if ($email) {
		...
		if (NULL == $emailid = exists_email($email))
			$emailid = Query("INSERT INTO `emails` (`email`, `emailtype`) 
			VALUES ( '" . $email . "','" . $emailtype . "' )", debug_backtrace());
	}
	return $emailid;
}
...
function insert_website($website = "", $websitetype = "") {
	...
	if ($website) {
		...
		if (NULL == $websiteid = exists_website($website))
			$websiteid = Query("INSERT INTO `websites` (`website`, `websitetype`) 
			VALUES ('" . $website . "','" . $websitetype . "')", debug_backtrace());
	}
	return $websiteid;
}
